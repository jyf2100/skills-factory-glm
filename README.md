...
| `GITHUB_URL` | GitHub 仓库 URL（用于同步） | - |
| `GITHUB_TOKEN` | GitHub Personal Access Token | - |
| `GITEA_URL` | Gitea 仓库 URL（远程同步） | - |
| `GITEA_TOKEN` | Gitea 认证令牌 | - |
...
Skills Factory 提供完整的 Docker 部署方案，包括：

- Skills Factory Web + API 服务
- GitLab 实例（技能数据源）
...
GITLAB_SKILLS_PROJECT=skills/skills
...
1. 构建镜像：
```bash
docker build -t skills-factory:latest .
```
...
Skills Factory 从本地 GitLab 读取技能数据。
...
# 在 GitLab 中生成：Settings -> Access Tokens
GITLAB_TOKEN=your-token-here

# GitLab skills project path
GITLAB_SKILLS_PROJECT=my-org/skills
```
...
- GitLab Token 需要具有 `api` 和 `read_repository` 权限
